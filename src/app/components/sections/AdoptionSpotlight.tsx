...
const ADOPTION_PETS = [
  { id: 'a1', name: 'Nala', breed: 'Labrador mix', age: '2 ans', description: 'Douce, affectueuse et excellente avec les enfants. Elle attend sa famille depuis 8 mois.', image: 'https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=600&h=450&fit=crop', city: 'Tunis', waitingSince: '8 mois', color: 'from-amber-400/20 to-orange-400/10' },
  { id: 'a2', name: 'Simba', breed: 'Chat européen roux', age: '1 an', description: 'Joueur et câlin, Simba cherche un foyer chaleureux et des bras accueillants.', image: 'https://images.unsplash.com/photo-1574158622682-e40e69881006?w=600&h=450&fit=crop', city: 'Sfax', waitingSince: '3 mois', color: 'from-emerald-400/20 to-teal-400/10' },
  { id: 'a3', name: 'Cookie', breed: 'Berger croisé', age: '3 ans', description: 'Sauvé de la rue, Cookie est maintenant prêt pour une deuxième chance. Propre et vacciné.', image: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600&h=450&fit=crop', city: 'Sousse', waitingSince: '5 mois', color: 'from-blue-400/20 to-indigo-400/10' },
];

export function AdoptionSpotlight({ onNavigate }: { onNavigate?: (page: string, params?: Record<string,string>) => void } = {}) {
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-14">
          ...
          >
            ...
            <span className="font-bold uppercase tracking-wider" style={{ fontSize: '12px' }}>{t('adoption.badge')}</span>
          </motion.div>
          ...
          <motion.h2
            ...
            style={{ fontFamily: 'Sora, sans-serif', fontSize: 'clamp(26px, 4vw, 44px)', fontWeight: 800, lineHeight: 1.1 }}
              ...
              <div className="relative overflow-hidden h-60">
                ...
                <div className="absolute inset-0 bg-[var(--pc-primary)]/80 opacity-0 group-hover:opacity-100 transition-all duration-400 flex items-center justify-center">
                  <div className="flex flex-col items-center gap-2 text-white scale-90 group-hover:scale-100 transition-transform duration-300">
                    ...
                    <span className="font-black" style={{ fontFamily: 'Sora, sans-serif', fontSize: '18px' }}>{t('adoption.adopt')}</span>
                  </div>
                </div>
                ...
                <div className="absolute top-3 left-3 flex items-center gap-1.5 glass-card px-3 py-1.5 rounded-full border border-white/30">
                  ...
                  <span className="text-white font-semibold" style={{ fontSize: '11px' }}>{t('adoption.waiting')} {pet.waitingSince}</span>
                </div>

                {/* Name on image bottom */}
                <div className="absolute bottom-3 left-3 right-3">
                  <h3 className="text-white font-black" style={{ fontFamily: 'Sora, sans-serif', fontSize: '22px' }}>{pet.name}</h3>
                  <p className="text-white/80" style={{ fontSize: '13px' }}>{pet.breed} · {pet.age} · {pet.city}</p>
                </div>
              </div>

              {/* Body */}
              <div className="p-4">
                <p className="text-[var(--pc-text-secondary)] leading-relaxed mb-4" style={{ fontSize: '13px' }}>
                  {pet.description}
                </p>
                <motion.button
