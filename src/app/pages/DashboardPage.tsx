...
type MainTab = 'overview' | 'myListings' | 'myFavorites' | 'messages' | 'notifications' | 'settings';
type SettingsSubTab = 'profile' | 'security' | 'appearance' | 'language';
type ListingStatus = 'active' | 'paused' | 'sold' | 'adopted' | 'expired' | 'pending';
...
const MOCK_LISTINGS_BASE: ListingItem[] = [
  { id: 'l1', title: 'Max - Berger Allemand', type: 'vente', price: '850 DT', city: 'Tunis', image: 'https://images.unsplash.com/photo-1552053831-71594a27632d?w=400&h=250&fit=crop', views: 1240, favorites: 34, messages: 5, status: 'active', daysLeft: 12 },
  { id: 'l2', title: 'Luna - Chatte tigrée', type: 'adoption', price: 'Gratuit', city: 'Sfax', image: 'https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?w=400&h=250&fit=crop', views: 890, favorites: 22, messages: 8, status: 'paused', daysLeft: 0 },
  { id: 'l3', title: 'Rocky - Lapin Angora', type: 'vente', price: '120 DT', city: 'Sousse', image: 'https://images.unsplash.com/photo-1585110396000-c9ffd4e4b308?w=400&h=250&fit=crop', views: 456, favorites: 11, messages: 2, status: 'sold', daysLeft: 0 },
  { id: 'l4', title: 'Coco - Spitz nain', type: 'perdu', price: '-', city: 'Monastir', image: 'https://images.unsplash.com/photo-1574144611937-0df059b5ef3e?w=400&h=250&fit=crop', views: 2100, favorites: 89, messages: 15, status: 'active', daysLeft: 5 },
];

const MOCK_FAVORITES: ListingItem[] = [
  { id: 'f1', title: 'Bella - Golden Retriever', type: 'adoption', price: 'Gratuit', city: 'Tunis', image: 'https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=400&h=250&fit=crop', views: 3200, favorites: 145, messages: 0, status: 'active', daysLeft: 20 },
  { id: 'f2', title: 'Milo - Perroquet Ara', type: 'vente', price: '2500 DT', city: 'Bizerte', image: 'https://images.unsplash.com/photo-1544473244-f84d3929e4a5?w=400&h=250&fit=crop', views: 780, favorites: 43, messages: 0, status: 'active', daysLeft: 14 },
];

const MOCK_NOTIFICATIONS_DATA: NotificationItem[] = [
  { id: 'n1', type: 'message', icon: '💬', text: 'Ahmed Sfax vous a envoyé un message', time: '2min', read: false },
  { id: 'n2', type: 'view', icon: '👁️', text: 'Votre annonce "Max" a été consultée 45 fois', time: '1h', read: false },
  { id: 'n3', type: 'follower', icon: '👤', text: 'Dr. Mehdi vous suit maintenant', time: '3h', read: false },
  { id: 'n4', type: 'like', icon: '❤️', text: 'Ines Monastir a aimé votre annonce', time: '1j', read: true },
  { id: 'n5', type: 'expiring', icon: '⏰', text: 'Votre annonce "Luna" expire dans 3 jours', time: '2j', read: true },
];

const MOCK_CONVERSATIONS = [
  { id: 'c1', name: 'Ahmed Sfax', avatar: 'AS', lastMessage: 'Est-ce que Max est encore disponible?', time: '2min', unread: 3 },
  { id: 'c2', name: 'Dr. Mehdi Ben Ali', avatar: 'MB', lastMessage: 'Merci pour les informations!', time: '1h', unread: 0 },
  { id: 'c3', name: 'Ines Monastir', avatar: 'IM', lastMessage: 'Je suis intéressée par Luna 🐱', time: '3h', unread: 1 },
];

const RECENT_ACTIVITY = [
  { icon: '👁️', text: 'Votre annonce "Max" a reçu 12 vues', time: '5min' },
  { icon: '❤️', text: 'Quelqu\'un a mis "Luna" en favoris', time: '20min' },
  { icon: '💬', text: 'Nouveau message d\'Ahmed Sfax', time: '1h' },
  { icon: '📋', text: 'Votre annonce "Rocky" a été mise en pause', time: '2h' },
  { icon: '🔔', text: 'Dr. Mehdi vous suit maintenant', time: '3h' },
];

const STATS = [
  { icon: '📋', label: 'Annonces actives', value: 4, trend: +2, color: 'emerald' },
  { icon: '⏸️', label: 'Annonces en pause', value: 1, trend: 0, color: 'amber' },
  { icon: '💰', label: 'Annonces vendues', value: 7, trend: +1, color: 'blue' },
  { icon: '💚', label: 'Adoptions réussies', value: 2, trend: +1, color: 'pink' },
  { icon: '👁️', label: 'Vues totales', value: 8420, trend: +340, color: 'purple' },
  { icon: '💬', label: 'Messages reçus', value: 23, trend: +5, color: 'indigo' },
  { icon: '❤️', label: 'Favoris reçus', value: 156, trend: +18, color: 'red' },
  { icon: '🔍', label: 'Vues du profil', value: 340, trend: +42, color: 'teal' },
];

const STATUS_BADGES: Record<ListingStatus, { label: string; color: string; dot: string }> = {
...
const AREA_DATA = Array.from({ length: 30 }, (_, i) => ({
  ...
  views: Math.floor(80 + Math.sin(i * 0.4) * 80 + i * 4),
}));
...
function typeBadgeClass(type: string): string {
  switch (type) {
    case 'adoption': return 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300';
    case 'vente': return 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300';
    case 'perdu': return 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300';
    case 'trouve': return 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300';
  ...
  return (
    <div className="relative w-16 h-16 flex-shrink-0">
      <svg className="w-16 h-16 -rotate-90" viewBox="0 0 64 64">
        <circle cx="32" cy="32" r={r} fill="none" strokeWidth="4" className="stroke-[var(--pc-border)]" />
        <circle
      ...
      <div
        ...
        style={{ background: 'linear-gradient(135deg, var(--pc-primary), #15a870)', borderRadius: '50%', margin: 6 }}
      >
        AB
      </div>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {STATS.map((stat, i) => (
          ...
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${COLOR_MAP[stat.color] ?? ''}`}>
              {stat.icon}
            </div>
            <div>
              <div className="text-2xl font-bold text-[var(--pc-text-primary)]">{stat.value.toLocaleString()}</div>
              <div className="text-xs text-[var(--pc-text-secondary)] leading-tight mt-0.5">{stat.label}</div>
            </div>
            {stat.trend !== 0 && (
              <div className={`text-xs font-semibold flex items-center gap-0.5 ${stat.trend > 0 ? 'text-emerald-500' : 'text-red-500'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="glass-card rounded-2xl p-5">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={AREA_DATA} margin={{ top: 4, right: 4, left: -28, bottom: 0 }}>
              <defs>
                <linearGradient id="areaGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="var(--pc-primary)" stopOpacity={0.6} />
              ...
              <XAxis dataKey="day" tick={{ fontSize: 10, fill: 'var(--pc-text-secondary)' }} tickLine={false} axisLine={false} interval={4} />
              <YAxis tick={{ fontSize: 10, fill: 'var(--pc-text-secondary)' }} tickLine={false} axisLine={false} />
              <Tooltip
                contentStyle={{ background: 'var(--pc-surface)', border: '1px solid var(--pc-border)', borderRadius: 12, fontSize: 12 }}
                labelStyle={{ color: 'var(--pc-text-secondary)' }}
                itemStyle={{ color: 'var(--pc-primary)' }}
              />
              <Area type="monotone" dataKey="views" stroke="var(--pc-primary)" strokeWidth={2} fill="url(#areaGrad)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="glass-card rounded-2xl p-5">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={BAR_DATA} margin={{ top: 4, right: 4, left: -28, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--pc-border)" vertical={false} />
              <XAxis dataKey="day" tick={{ fontSize: 11, fill: 'var(--pc-text-secondary)' }} tickLine={false} axisLine={false} />
              <YAxis tick={{ fontSize: 10, fill: 'var(--pc-text-secondary)' }} tickLine={false} axisLine={false} />
              <Tooltip
                contentStyle={{ background: 'var(--pc-surface)', border: '1px solid var(--pc-border)', borderRadius: 12, fontSize: 12 }}
                labelStyle={{ color: 'var(--pc-text-secondary)' }}
                itemStyle={{ color: 'var(--pc-primary)' }}
              />
              <Bar dataKey="count" fill="var(--pc-primary)" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div className="space-y-0">
          {RECENT_ACTIVITY.map((item, i) => (
            ...
            >
              ...
              <p className="flex-1 text-sm text-[var(--pc-text-primary)] leading-snug">{item.text}</p>
              <span className="text-xs text-[var(--pc-text-secondary)] shrink-0">{item.time}</span>
            </motion.div>
          ))}
        </div>
      </div>
...
const MODAL_CONFIG: Record<string, { title: string; message: string; confirmLabel: string; variant: 'danger' | 'warning' | 'success' }> = {
  pause: { title: 'Mettre en pause ?', message: 'Votre annonce n\'apparaîtra plus dans les recherches.', confirmLabel: 'Mettre en pause', variant: 'warning' },
  resume: { title: 'Réactiver l\'annonce ?', message: 'Votre annonce sera de nouveau visible dans les recherches.', confirmLabel: 'Réactiver', variant: 'success' },
  sold: { title: 'Marquer comme vendu ?', message: 'L\'annonce sera archivée avec le badge "Vendu".', confirmLabel: 'Marquer vendu', variant: 'success' },
  adopted: { title: 'Marquer comme adopté ?', message: 'L\'annonce sera archivée avec le badge "Adopté".', confirmLabel: 'Marquer adopté', variant: 'success' },
  delete: { title: 'Supprimer définitivement ?', message: 'Cette action est irréversible. L\'annonce sera supprimée définitivement.', confirmLabel: 'Supprimer', variant: 'danger' },
  renew: { title: 'Renouveler l\'annonce ?', message: 'L\'annonce sera visible pour 30 jours supplémentaires.', confirmLabel: 'Renouveler', variant: 'success' },
};

function MyListingsTab({ onNavigate }: { onNavigate: (page: string, params?: Record<string, string>) => void }) {
  const [filter, setFilter] = useState<FilterKey>('all');
  const [listings, setListings] = useState<ListingItem[]>(MOCK_LISTINGS_BASE);
  const [modal, setModal] = useState<ModalState>({ type: null, listingId: null, loading: false });
  ...
  const closeModal = () => setModal({ type: null, listingId: null, loading: false });
  ...
  const handleConfirm = () => {
    ...
    setTimeout(() => {
      ...
      setListings((prev) => {
        ...
        if (type === 'pause') return prev.map((l) => l.id === id ? { ...l, status: 'paused' as ListingStatus } : l);
        if (type === 'resume') return prev.map((l) => l.id === id ? { ...l, status: 'active' as ListingStatus } : l);
        if (type === 'sold') return prev.map((l) => l.id === id ? { ...l, status: 'sold' as ListingStatus } : l);
        if (type === 'adopted') return prev.map((l) => l.id === id ? { ...l, status: 'adopted' as ListingStatus } : l);
        if (type === 'renew') return prev.map((l) => l.id === id ? { ...l, daysLeft: 30, status: 'active' as ListingStatus } : l);
        return prev;
      });
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-semibold text-[var(--pc-text-primary)]">
          Mes annonces{' '}
          <span className="text-[var(--pc-text-secondary)] font-normal text-sm">({listings.length})</span>
        </h2>
        <motion.button
      ...
      {visible.length === 0 ? (
        <div className="glass-card rounded-2xl p-12 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-[var(--pc-text-secondary)] text-sm">Aucune annonce dans cette catégorie</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {visible.map((listing) => (
            <ListingCard
              ...
              onPause={() => openModal(listing.status === 'active' ? 'pause' : 'resume', listing.id)}
...
function ListingCard({ listing, onView, onPause, onSold, onAdopted, onDelete, onDuplicate, onRenew }: ListingCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        />
        <span className={`absolute top-2 left-2 ${badge.color} text-white text-xs px-2 py-0.5 rounded-full font-semibold shadow`}>
      ...
      <div className="flex items-center gap-4 px-3 py-2 bg-[var(--pc-surface-alt)] text-xs text-[var(--pc-text-secondary)]">
        <span className="flex items-center gap-1"><Eye size={11} /> {listing.views.toLocaleString()}</span>
        <span className="flex items-center gap-1"><Heart size={11} /> {listing.favorites}</span>
        <span className="flex items-center gap-1"><MessageCircle size={11} /> {listing.messages}</span>
      </div>
      ...
      <div className="p-3 flex flex-col gap-1.5 flex-1">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className={`shrink-0 text-xs px-2 py-0.5 rounded-full font-medium ${typeBadgeClass(listing.type)}`}>
            {listing.type}
          </span>
        </div>
        <div className="flex items-center justify-between text-xs text-[var(--pc-text-secondary)]">
          <span>📍 {listing.city}</span>
          <span className="font-semibold text-[var(--pc-text-primary)]">{listing.price}</span>
        </div>
        {expiresWarn && (
      ...
      <div className="px-3 pb-3 grid grid-cols-3 gap-1.5">
        ...
        <button
          className="flex items-center justify-center gap-1 text-xs py-1.5 rounded-lg bg-[var(--pc-primary)]/10 text-[var(--pc-primary)] hover:bg-[var(--pc-primary)]/20 transition-colors font-medium"
        >
...
function MyFavoritesTab({ onNavigate }: { onNavigate: (page: string, params?: Record<string, string>) => void }) {
  ...
  if (favorites.length === 0) {
    return (
      <div className="glass-card rounded-2xl p-12 flex flex-col items-center gap-4 text-center">
        ...
        <div>
          <h3 className="font-semibold text-[var(--pc-text-primary)] mb-1">Aucun favori</h3>
          <p className="text-sm text-[var(--pc-text-secondary)]">Explorez les annonces et ajoutez-les à vos favoris.</p>
        </div>
        <button
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-semibold text-[var(--pc-text-primary)]">
          Mes favoris{' '}
          <span className="text-[var(--pc-text-secondary)] font-normal text-sm">({favorites.length})</span>
        </h2>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
...
function MessagesTab({ onNavigate }: { onNavigate: (page: string) => void }) {
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-semibold text-[var(--pc-text-primary)]">Messages</h2>
        <button
          onClick={() => onNavigate('messages')}
        ...
        {MOCK_CONVERSATIONS.map((conv, i) => (
          <motion.button
            ...
            className={`w-full flex items-center gap-3 p-4 text-left transition-colors ${
              i < MOCK_CONVERSATIONS.length - 1 ? 'border-b border-[var(--pc-border)]' : ''
            }`}
          >
            <div
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-0.5">
                <span className="text-sm font-semibold text-[var(--pc-text-primary)]">{conv.name}</span>
                <span className="text-xs text-[var(--pc-text-secondary)]">{conv.time}</span>
              </div>
              <p className={`text-sm truncate ${conv.unread > 0 ? 'text-[var(--pc-text-primary)] font-medium' : 'text-[var(--pc-text-secondary)]'}`}>
                {conv.lastMessage}
              </p>
            </div>
            {conv.unread > 0 && (
            ...
            <ChevronRight size={14} className="text-[var(--pc-text-secondary)] shrink-0" />
          </motion.button>
        ))}
      </div>
    </div>
        ...
        {notifications.map((notif, i) => (
          <motion.div
            ...
            className={`flex items-start gap-3 p-4 ${
              i < notifications.length - 1 ? 'border-b border-[var(--pc-border)]' : ''
            } ${!notif.read ? 'bg-[var(--pc-primary)]/5' : ''}`}
          >
            <span className="text-xl leading-none mt-0.5">{notif.icon}</span>
            <div className="flex-1 min-w-0">
              <p className={`text-sm leading-snug ${notif.read ? 'text-[var(--pc-text-secondary)]' : 'text-[var(--pc-text-primary)] font-medium'}`}>
                {notif.text}
              </p>
              <span className="text-xs text-[var(--pc-text-secondary)] mt-0.5 block">{notif.time}</span>
            </div>
            {!notif.read && (
              <span className="w-2 h-2 rounded-full mt-1.5 shrink-0" style={{ background: 'var(--pc-primary)' }} />
            )}
          </motion.div>
        ))}
      </div>
...
function SettingsTab() {
  ...
  const subTabs: { key: SettingsSubTab; icon: React.ReactNode; label: string }[] = [
  ...
  return (
    <div>
      <h2 className="font-semibold text-[var(--pc-text-primary)] mb-4">Paramètres</h2>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={subTab} variants={tabVariants} initial="initial" animate="animate" exit="exit">
          ...
          {subTab === 'profile' && (
            <div className="glass-card rounded-2xl p-5 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-semibold text-[var(--pc-text-primary)]">{profileForm.firstName} {profileForm.lastName}</p>
                  <p className="text-xs text-[var(--pc-text-secondary)]">Changer la photo</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {profileFields.map(({ key, label }) => (
                  <div key={key}>
                    <label className="block text-xs font-medium text-[var(--pc-text-secondary)] mb-1">{label}</label>
                    <input
              ...
              <div>
                <label className="block text-xs font-medium text-[var(--pc-text-secondary)] mb-1">Bio</label>
                <textarea
          ...
          {subTab === 'appearance' && (
            <div className="glass-card rounded-2xl p-5 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                {(
                  [
                    { key: 'light' as const, icon: <Sun size={20} />, label: 'Clair' },
                    { key: 'dark' as const, icon: <Moon size={20} />, label: 'Sombre' },
                    { key: 'system' as const, icon: <Monitor size={20} />, label: 'Système' },
                  ]
                ).map((opt) => (
                  <motion.button
                    key={opt.key}
                  ...
                  >
                    ...
                    {theme === opt.key && (
                      <span className="w-2 h-2 rounded-full" style={{ background: 'var(--pc-primary)' }} />
                    )}
                  </motion.button>
                ))}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className={`text-sm font-semibold ${isActive ? 'text-[var(--pc-primary)]' : 'text-[var(--pc-text-primary)]'}`}>
                          {lang.native}
                        </p>
                        <p className="text-xs text-[var(--pc-text-secondary)]">{lang.label}</p>
                      </div>
                      {isActive && <CheckCircle size={15} className="text-[var(--pc-primary)] shrink-0" />}
                    </motion.button>
                  );
                })}
              </div>
...
export function DashboardPage({ onBack, onNavigate }: DashboardPageProps) {
  ...
  const NAV_TABS: NavTab[] = [
    { key: 'overview', icon: <LayoutDashboard size={18} />, label: 'Vue d\'ensemble' },
    { key: 'myListings', icon: <List size={18} />, label: 'Mes annonces', badge: 4 },
    { key: 'myFavorites', icon: <Heart size={18} />, label: 'Favoris', badge: 2 },
    { key: 'messages', icon: <MessageCircle size={18} />, label: 'Messages', badge: 4 },
    { key: 'notifications', icon: <Bell size={18} />, label: 'Notifications', badge: 3 },
    ...
    <div className="min-h-screen bg-[var(--pc-surface-alt)] flex">
        ...
        <div className="p-5 border-b border-[var(--pc-border)]">
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0 pt-1">
              <p className="font-bold text-[var(--pc-text-primary)] text-sm leading-tight">Ahmed Ben Salah</p>
              <p className="text-xs text-[var(--pc-text-secondary)] truncate">ahmed@email.com</p>
          ...
          <div className="flex items-center justify-between text-xs">
            <span className="text-[var(--pc-text-secondary)]">Profil complété à 75%</span>
          </div>
          <div className="w-full h-1.5 rounded-full bg-[var(--pc-border)] mt-2 overflow-hidden">
            <div className="h-full rounded-full" style={{ width: '75%', background: 'var(--pc-primary)' }} />
          </div>
        ...
        <nav className="flex-1 p-3 space-y-1">
          {NAV_TABS.map((tab) => (
            <button
              ...
              style={activeTab === tab.key ? { background: 'var(--pc-primary)' } : {}}
            >
              {tab.icon}
              <span className="flex-1">{tab.label}</span>
              {tab.badge !== undefined && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${
                  activeTab === tab.key ? 'bg-white/20 text-white' : 'bg-[var(--pc-primary)]/15 text-[var(--pc-primary)]'
                }`}>
                  {tab.badge}
                </span>
              )}
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <header className="lg:hidden sticky top-0 z-10 bg-[var(--pc-surface)] border-b border-[var(--pc-border)] px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="font-bold text-[var(--pc-text-primary)] text-sm">Mon tableau de bord</p>
            <p className="text-xs text-[var(--pc-text-secondary)]">Ahmed Ben Salah</p>
          </div>
            ...
            {NAV_TABS.map((tab) => (
              <button
                ...
                className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                  activeTab === tab.key ? 'text-white' : 'text-[var(--pc-text-secondary)] bg-[var(--pc-surface-alt)] hover:text-[var(--pc-text-primary)]'
                }`}
                style={activeTab === tab.key ? { background: 'var(--pc-primary)' } : {}}
              >
                {tab.icon}
                {tab.label}
                {tab.badge !== undefined && (
                  <span className={`text-xs px-1 rounded-full font-bold ${activeTab === tab.key ? 'bg-white/25 text-white' : 'bg-[var(--pc-primary)]/15 text-[var(--pc-primary)]'}`}>
                    {tab.badge}
                  </span>
                )}
              </button>
          ...
          <div className="max-w-4xl mx-auto">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {activeTab === 'myListings' && <MyListingsTab onNavigate={onNavigate} />}
                {activeTab === 'myFavorites' && <MyFavoritesTab onNavigate={onNavigate} />}
