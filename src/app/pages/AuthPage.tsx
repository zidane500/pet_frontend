...
type AuthView = 'login' | 'register-step1' | 'register-step2' | 'forgot' | 'reset';
...
const ROLES = [
  { id: 'owner', icon: '🐾', title: 'Propriétaire / Visiteur', desc: 'Achetez, adoptez, signalez des animaux perdus', color: 'from-emerald-500 to-teal-600' },
  { id: 'vet', icon: '🏥', title: 'Vétérinaire', desc: 'Gérez votre profil et vos consultations', color: 'from-blue-500 to-indigo-600' },
  { id: 'shop', icon: '🏪', title: 'Animalerie', desc: 'Publiez vos produits et services', color: 'from-amber-500 to-orange-600' },
  { id: 'shelter', icon: '🏠', title: 'Refuge / SPA', desc: 'Gérez les adoptions et recevez des dons', color: 'from-pink-500 to-rose-600' },
  { id: 'breeder', icon: '🐕', title: 'Éleveur', desc: 'Publiez vos animaux et gérez vos portées', color: 'from-purple-500 to-violet-600' },
];
...
function InputField({ label, type = 'text', value, onChange, placeholder, icon: Icon, error, suffix, disabled }: InputFieldProps) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-[var(--pc-text-primary)] font-semibold text-sm block">{label}</label>
      <div className="relative flex items-center">
        ...
        <input
          ...
          className={[
            ...
            Icon ? 'pl-9' : '',
            suffix ? 'pr-10' : '',
            error ? 'border-red-400 focus:ring-red-400/30' : 'border-[var(--pc-border)]',
...
// ─── PREMIUM Left branding panel with LOGO + ANIMATIONS ───────────────────────
...
function LeftPanel() {
  ...
  return (
    <div className="hidden lg:flex flex-col items-center justify-center h-full relative overflow-hidden bg-[#0A1F1A]">
      ...
      {/* Floating orbs */}
      <FloatingOrb delay={0} size={300} x="10%" y="20%" color="bg-[#2D90BA]/20" />
      <FloatingOrb delay={2} size={250} x="60%" y="60%" color="bg-[#55D2E1]/15" />
      <FloatingOrb delay={4} size={200} x="30%" y="70%" color="bg-[#1D7D5F]/20" />
      <FloatingOrb delay={1} size={180} x="70%" y="10%" color="bg-[#55D2E1]/10" />
      <FloatingOrb delay={3} size={220} x="-5%" y="50%" color="bg-[#2D90BA]/15" />
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        >
          ...
          <motion.div
            ...
            style={{
              background: 'radial-gradient(circle, rgba(85,210,225,0.3) 0%, transparent 70%)',
              transform: 'scale(1.5)',
            }}
          />
          ...
          <motion.div
            ...
            animate={{
              ...
              opacity: [0.05, 0.1, 0.04, 0.08, 0.05],
            }}
            transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
            style={{
              background: 'radial-gradient(circle, rgba(45,144,186,0.3) 0%, transparent 70%)',
              transform: 'scale(2)',
            }}
          />
          ...
          >
            <motion.img
              src="/public/animali-logo.png"
              alt="Animali.tn Logo"
              className="w-full h-full object-contain drop-shadow-2xl"
              animate={glowVariants.animate}
              style={{
                filter: 'drop-shadow(0 0 30px rgba(85, 210, 225, 0.3)) drop-shadow(0 0 60px rgba(45, 144, 186, 0.2))',
              }}
            />
          </motion.div>
        </motion.div>
        ...
        >
          ...
          {/* Subtle tagline with typewriter effect */}
          <motion.p
            custom={1}
            variants={textVariants}
            className="mt-4 text-white/40 text-sm font-medium tracking-[0.3em] uppercase"
          >
            {"La plateforme des passionnés d'animaux".split('').map((char, i) => (
              <motion.span
                key={i}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
        ...
        <motion.div
          className="absolute top-1/4 left-1/4 w-2 h-2 rounded-full bg-[#55D2E1]/40"
          animate={{
            scale: [1, 2, 1],
            opacity: [0.4, 0.1, 0.4],
          }}
        ...
        />
        <motion.div
          className="absolute bottom-1/3 right-1/4 w-1.5 h-1.5 rounded-full bg-[#2D90BA]/40"
          animate={{
            scale: [1, 1.5, 1],
            opacity: [0.3, 0.6, 0.3],
          }}
        ...
        />
        <motion.div
          className="absolute top-1/2 right-1/3 w-1 h-1 rounded-full bg-white/20"
          animate={{
            scale: [1, 3, 1],
            opacity: [0.2, 0, 0.2],
          }}
  ...
  return (
    <div className="space-y-5">
      <div className="text-center mb-6">
        <h1 className="text-2xl font-black text-[var(--pc-text-primary)]">Bon retour !</h1>
        <p className="text-[var(--pc-text-secondary)] text-sm mt-1">Connectez-vous à votre compte Animali.tn</p>
      </div>
      ...
      <div className="relative flex items-center gap-3 my-2">
        ...
        <span className="text-[var(--pc-text-secondary)] text-xs">ou continuer avec</span>
        <div className="flex-1 h-px bg-[var(--pc-border)]" />
      </div>
      ...
      <div className="space-y-3">
        <button
          type="button"
          className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-[var(--pc-border)]
            bg-[var(--pc-surface-alt)] text-[var(--pc-text-primary)] text-sm font-medium
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-[var(--pc-border)]
            bg-[var(--pc-surface-alt)] text-[var(--pc-text-primary)] text-sm font-medium
            hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-300 hover:text-blue-600 transition-all"
        >
  ...
  return (
    <div className="space-y-5">
      <div className="text-center mb-4">
        <h1 className="text-2xl font-black text-[var(--pc-text-primary)]">Créer votre compte</h1>
        <p className="text-[var(--pc-text-secondary)] text-sm mt-1">Étape 1/2 — Choisissez votre type de compte</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {ROLES.map((role, idx) => {
          ...
          return (
            ...
            >
              <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${role.color} flex items-center justify-center text-lg mb-3`}>
                {role.icon}
              </div>
              <p className="font-bold text-[var(--pc-text-primary)] text-sm leading-snug">{role.title}</p>
              <p className="text-[var(--pc-text-secondary)] text-xs mt-0.5 leading-snug">{role.desc}</p>
              {selected && (
      ...
      <GradientButton onClick={() => onSwitch('register-step2')} disabled={!selectedRole}>
        Continuer →
      </GradientButton>
...
function RegisterStep2View({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (confirmPassword !== password) newErrors.confirm = 'Les mots de passe ne correspondent pas';
    if (!acceptTerms) newErrors.terms = "Veuillez accepter les conditions d'utilisation";
    if (!acceptPrivacy) newErrors.privacy = 'Veuillez accepter la politique de confidentialité';
    setErrors(newErrors);
    if (Object.keys(newErrors).length > 0) return;
  ...
  }, [firstName, lastName, username, email, password, confirmPassword, acceptTerms, acceptPrivacy, selectedRole, onSuccess]);

  return (
    <div className="space-y-4">
      <div className="text-center mb-2">
        <h1 className="text-2xl font-black text-[var(--pc-text-primary)]">Créer votre compte</h1>
        <p className="text-[var(--pc-text-secondary)] text-sm mt-1">Étape 2/2 — Informations personnelles</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <InputField label="Prénom" value={firstName} onChange={setFirstName} placeholder="Jean" icon={User} error={errors.firstName} />
        <InputField label="Nom" value={lastName} onChange={setLastName} placeholder="Dupont" icon={User} error={errors.lastName} />
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-[var(--pc-text-primary)] font-semibold text-sm block">Téléphone</label>
        <div className="relative flex items-center">
          <Phone size={16} className="absolute left-3 text-[var(--pc-text-secondary)] pointer-events-none z-10" />
      ...
      <div className="flex flex-col gap-1.5">
        <InputField
          ...
          error={errors.password}
          suffix={
            <button type="button" onClick={() => setShowPassword((p) => !p)} className="text-[var(--pc-text-secondary)] hover:text-[var(--pc-text-primary)]">
              {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
            </button>
          }
        />
        {password.length > 0 && (
          <div className="space-y-1 mt-1">
            ...
            <p className="text-xs text-[var(--pc-text-secondary)]">{strength > 0 ? strengthLabels[strength - 1] : 'Trop court'}</p>
          </div>
        )}
      </div>
      ...
      <InputField
        ...
        suffix={
          <button type="button" onClick={() => setShowConfirm((p) => !p)} className="text-[var(--pc-text-secondary)] hover:text-[var(--pc-text-primary)]">
      ...
      <div className="space-y-2">
        <label className="flex items-start gap-2.5 cursor-pointer">
          ...
          <span className="text-sm text-[var(--pc-text-secondary)]">
            J'accepte les{' '}
            <span className="text-[#1D7D5F] font-medium cursor-pointer hover:underline">Conditions d'utilisation</span>
          </span>
        </label>
        {errors.terms && <p className="text-red-400 text-xs pl-6">{errors.terms}</p>}
        ...
        <label className="flex items-start gap-2.5 cursor-pointer">
          ...
          <span className="text-sm text-[var(--pc-text-secondary)]">
            J'accepte la{' '}
            <span className="text-[#1D7D5F] font-medium cursor-pointer hover:underline">Politique de confidentialité</span>
          </span>
        </label>
        {errors.privacy && <p className="text-red-400 text-xs pl-6">{errors.privacy}</p>}
      </div>
...
function ForgotView({ onSwitch }: { onSwitch: (v: AuthView) => void }) {
  ...
  if (sent) {
    return (
      ...
      >
        ...
        <div>
          <h2 className="text-xl font-black text-[var(--pc-text-primary)]">Email envoyé !</h2>
  ...
  return (
    <div className="space-y-5">
      <div className="text-center mb-4">
        <h1 className="text-2xl font-black text-[var(--pc-text-primary)]">Mot de passe oublié</h1>
...
function ResetView({ onSwitch }: { onSwitch: (v: AuthView) => void }) {
  ...
  if (success) {
    return (
      ...
      >
        ...
        <div>
          <h2 className="text-xl font-black text-[var(--pc-text-primary)]">Mot de passe changé avec succès !</h2>
          <p className="text-[var(--pc-text-secondary)] text-sm mt-2">Redirection vers la connexion…</p>
        </div>
  ...
  return (
    <div className="space-y-5">
      <div className="text-center mb-4">
        <h1 className="text-2xl font-black text-[var(--pc-text-primary)]">Réinitialiser le mot de passe</h1>
        <p className="text-[var(--pc-text-secondary)] text-sm mt-1">Choisissez un nouveau mot de passe sécurisé</p>
      </div>
...
export function AuthPage({ initialView = 'login', onSuccess, onNavigate: _onNavigate }: AuthPageProps) {
  const { i18n } = useTranslation();
  const isRtl = i18n.language === 'ar';

  const startView: AuthView = initialView === 'register' ? 'register-step1' : 'login';
  const [view, setView] = useState<AuthView>(startView);
  const [selectedRole, setSelectedRole] = useState('');
  const [prevView, setPrevView] = useState<AuthView>(startView);

  const viewOrder: AuthView[] = ['login', 'forgot', 'reset', 'register-step1', 'register-step2'];

  const handleSwitch = useCallback((next: AuthView) => {
    setPrevView(view);
    setView(next);
  }, [view]);

  const slideDir = viewOrder.indexOf(view) >= viewOrder.indexOf(prevView) ? 1 : -1;
  ...
  return (
    <div dir={isRtl ? 'rtl' : 'ltr'} className="min-h-screen flex bg-[var(--pc-surface)]">
      {/* Left branding panel - PREMIUM with LOGO + ANIMATIONS */}
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-10 overflow-y-auto min-h-screen">
        <div className="glass-card max-w-md w-full p-8 rounded-2xl">
          ...
          <AnimatePresence mode="wait" custom={slideDir}>
            ...
            >
              {view === 'login' && (
                <LoginView onSwitch={handleSwitch} onSuccess={onSuccess} />
              )}
              {view === 'register-step1' && (
                <RegisterStep1View
              ...
              {view === 'reset' && (
                <ResetView onSwitch={handleSwitch} />
              )}
            </motion.div>
          </AnimatePresence>
        </div>
